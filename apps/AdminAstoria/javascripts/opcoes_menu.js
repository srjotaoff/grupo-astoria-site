...
async function saveCard(card) {
  ...
  try {
    var url    = isNew ? BASE + '/admin/opcoes' : BASE + '/admin/opcoes/' + id
...
async function deleteCard(card) {
  ...
  try {
    var res = await fetch(BASE + '/admin/opcoes/' + id, { method: 'DELETE', credentials: 'include' })
    if (res.status === 401 || res.status === 403) { window.location.replace(BASE + '/'); return }
    if (!res.ok) {
      var data = await res.json().catch(function () { return {} })
...
async function loadOpcoes() {
  try {
    var res  = await fetch(BASE + '/admin/opcoes', { credentials: 'include' })
    if (res.status === 401 || res.status === 403) { window.location.replace(BASE + '/'); return }
